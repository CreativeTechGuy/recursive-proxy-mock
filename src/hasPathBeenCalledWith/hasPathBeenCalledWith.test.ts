...
describe("hasPathBeenCalledWith", () => {
    test("returns false and console.warn when argument isn't a proxy mock", () => {
        ...
        expect(hasPathBeenCalledWith(null, [], [])).toBe(false);
        expect(consoleWarnSpy).toHaveBeenCalled();
    });

    test("returns false and console.warn when path doesn't contain APPLY or CONSTRUCT", () => {
        const consoleWarnSpy = jest.spyOn(console, "warn").mockImplementation();
        expect(hasPathBeenCalledWith(recursiveProxyMock(), [], [])).toBe(false);
    ...
    test("returns true if any calls matches", () => {
        ...
        proxy.a.b(5, true);
        expect(hasPathBeenCalledWith(proxy, ["a", "b", ProxySymbol.APPLY], ["a", 12, false])).toBe(true);
    });
    ...
    test("returns false if no calls match", () => {
        ...
        proxy.a.b("a", 12, false);
        proxy.a.b(5, true);
        expect(hasPathBeenCalledWith(proxy, ["a", "b", ProxySymbol.APPLY], [12])).toBe(false);
    });
    ...
    test("returns true if any constructor calls matches", () => {
        ...
        expect(hasPathBeenCalledWith(proxy, ["a", "b", ProxySymbol.CONSTRUCT], ["a", 12, false])).toBe(true);
    });
    ...
    test("returns false if no constructor calls match", () => {
        ...
        new proxy.a.b("a", 12, false);
        new proxy.a.b(5, true);
        expect(hasPathBeenCalledWith(proxy, ["a", "b", ProxySymbol.CONSTRUCT], [12])).toBe(false);
    });

    test("returns false if constructor was unused", () => {
        const proxy = recursiveProxyMock();
        expect(hasPathBeenCalledWith(proxy, ["a", ProxySymbol.APPLY], [12])).toBe(false);
    });
});
